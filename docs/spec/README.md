---
title: Specification Index
document_type: navigation
classification: confirmed fact
status: Mixed approved baseline and proposal-only contract documentation
last_verified: 2026-07-29
related_documents: ["../discovery/decisions.md","ux/README.md","traceability-implementation.md","traceability-ux-implementation.md","actor-authorization-contract.md","lifecycle-contract.md"]
decision_authority: decisions.md and Issue #7 proposal-only documentation approval
---

# Specification Index

## Approved product and policy SOT

- [MVP scope](mvp-scope.md)
- [Session experience](session-experience.md)
- [Game/content system](game-content-system.md)
- [Progressive disclosure](progressive-disclosure.md)
- [Matching/progression](matching-and-progression.md)
- [Invitations/attendance](invitations-and-attendance.md)
- [Web/mobile non-functional gates](web-mobile-experience.md)
- [Non-functional requirements](non-functional-requirements.md)
- [Trust/safety principles](trust-safety-moderation.md)
- [Admin/operations capabilities](admin-and-operations.md)
- [Identity/admission principles](security/identity-admission-and-invitations.md)
- [Retention policy](data/retention-matrix.md)

Approved rules do not imply approved screen behavior or implementation.

## Post-D-024 contract-gated proposals

- [Approved UX baseline](ux/README.md)
- [API capability inventory](api/README.md)
- [Real-time capability boundary](api/realtime-capabilities.md)
- [Conceptual data preparation](data/README.md)
- [Conceptual domain data model](data/domain-data-model.md)
- [Implementation traceability gate](traceability-implementation.md)
- [Implementation Contract phase proposal](../discovery/implementation-contract-promotion-proposal.md)
- [UX-to-Implementation traceability matrix](traceability-ux-implementation.md)
- [Actor and authorization contract proposal](actor-authorization-contract.md)
- [Lifecycle contract proposal](lifecycle-contract.md)

D-024 is satisfied and the prototype validation is complete. The owner approved
proposal-only Implementation Contract documentation in Issue #7. Every
contract-facing artifact remains `classification: proposal` and
`implementation_ready: false`. No endpoint-level OpenAPI, AsyncAPI, DBML,
migration, final state enum, page authorization or component contract is
authoritative.

## Historical journey and traceability drafts

Existing journey and traceability documents remain proposal material unless they directly link to an approved SOT. When they conflict, decisions.md and the approved SOT above control.

