...
Issue #70 pins requested `workspace-write`; Issue #72 requires a same-host executable/version/config/environment-bound write probe before every real patch-only Worker. It must create the exact workspace target, receive a machine-readable denied boundary write, and preserve sanitized raw JSONL/stdout/stderr, event inventory, usage, binding and filesystem evidence in an atomically finalized OS-temp directory before any usage, budget or sandbox verdict.
Issue #74 closes the evidence-loss path: successful, malformed, truncated, mismatched and over-budget probes retain evidence; a staging or rename failure is `RUNNER_PROBE_ARTIFACT_WRITE_FAILED` and no partial final directory is evidence. Usage-unverified precedes verified external-call and token budgets; verified budget usage is always aggregated before Worker launch, while effective mismatch remains `BLOCKED_ENVIRONMENT` when budget is within bounds.
`RUN-AH-P2-19-EXTERNAL-HOST-RUNBOOK-007` is immutable `FAILED_BUDGET`: preserved JSONL proves two distinct `node_repl` MCP calls for repository-instruction discovery, not parser duplication; `probe.txt` was created, but inline `try {...}; catch` produced `MissingCatchOrFinally`, so boundary denial stayed false and no Worker started. The probe must instead disable every discovered MCP server and web search, reject any MCP/web/extra command/file change as `RUNNER_CODEX_PROBE_TOOL_POLICY_VIOLATION`, and run one hash-bound Runner-owned PowerShell `-File` script with exact argv. Any later Pilot needs a new run ID, fresh exact Owner approval and residual-risk acceptance; product code, schema/migration, dependency/workflow and automatic publication remain prohibited.
