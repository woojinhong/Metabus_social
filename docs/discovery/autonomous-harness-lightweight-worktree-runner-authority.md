...
`RUN-AH-P2-17-EXTERNAL-HOST-RUNBOOK-006` retained only its manifest (`FAILED_BUDGET`, reported external calls 2), so the item types and whether the count represents calls or parser behavior are evidence unavailable and must not be inferred. An artifact-less usage number is not authoritative investigation evidence; the run is immutable and cannot be rerun. Any next Pilot waits for Issue #74 to merge and needs a new run ID and fresh exact Owner approval; product code, schema/migration, dependency/workflow and automatic publication remain prohibited.
