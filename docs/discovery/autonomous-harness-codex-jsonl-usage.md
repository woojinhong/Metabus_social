...
status: AH-P2-11 Issue #68 wiring in Draft-PR review; new Pilot still separately gated
...
Issue #68 permits unavailable cost only when the exact approved ChatGPT,
non-production, single-file docs-only patch policy is bound. Otherwise the
adapter returns `RUNNER_CODEX_COST_AUTHORITY_REQUIRED` before launch.
...
and binds the narrow monetary-policy pin. AH-P2-11 never infers that ChatGPT
authentication or `max_cost: 0` means zero monetary cost.
...
only for its exact scope; Issue #68 implements the post-run gate under review,
while a fresh run approval remains required.
...
no Pilot or Codex Worker was run for Issues #64 or #68.
