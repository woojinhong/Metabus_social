...
| Operations | `operations rehearsal required` | O01-O07 UX work-area boundaries and specified separation constraints are approved | Exact operator authority, staffing, coverage, readiness, training, handoff and tabletop evidence are unapproved | Owner approves the staffing/rehearsal package; staff roles, train and rehearse | Owner-approved coverage and independence proven; approved tabletops pass with audited evidence | Yes: proposed staffing design/training; final rehearsal after tools exist |
...
| Metadata/retention | [Agent insights](https://docs.livekit.io/deploy/observability/insights/) states a 30-day window, but Build/free model improvement may retain some anonymized session data longer. The [DPA](https://livekit.com/legal/data-processing-addendum) says observability/log data is US-processed regardless of pinned region and encrypted backups may persist up to 30 additional days. | Keep agent observability disabled; vendor must identify every intended metadata class, role, region, retention, deletion/export and backup expiry. |
...
Do not start live participation if any Gate lacks any applicable required
evidence, such as a named owner, dated evidence, approved contract/privacy
review, tested failure/cancellation path, quota alert, deletion proof or
incident contact. An outage or uncertain eligibility fails closed; it never
enables manual documents, recording, broader telemetry, mid-session vendor
migration or silent access widening.
